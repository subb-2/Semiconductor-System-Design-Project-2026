...
module i2c_master (
    ...
    // done_reg 래치 로직 (기존 always 블록과 별도로 추가)
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            done_reg <= 1'b0;
        end else begin
            if (cmd_write || cmd_read || cmd_stop || cmd_start) begin
                done_reg <= 1'b0;  // 새 명령 오면 클리어
            end else if (done) begin
                done_reg <= 1'b1;  // done 펄스 오면 래치
            end
        end
    end
